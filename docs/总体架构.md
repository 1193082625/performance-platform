...
MVP 0.1 先验证 FP/FCP 最小闭环，不部署完整数据平台组件：

```mermaid
flowchart LR
    DEMO[Demo / 业务页面] --> SDK[Web SDK: FP/FCP]
    SDK -->|POST /api/v1/events/batch| API[Node.js 单体 API]
    API --> PG[(PostgreSQL)]
    UI[Vue Console: 免登录] -->|GET /api/v1/metrics/paint| API
```
...
MVP 0.1 不包含登录、团队、项目管理、Redis、ClickHouse、独立接入网关、独立数据处理/计算服务、告警和诊断。以下章节描述 MVP 1.0 及后续目标架构。
